...
static void SimpleRandomTest(httplib::Client* cli);
static void PlotDoubleTest(httplib::Client* cli);
static void PlotRandomTest(httplib::Client* cli);
...
void TestHeatConductionSolver(httplib::Client* cli) {
  ...
  RUN_TEST_REMOTE(suite, cli, SimpleRandomTest);
  RUN_TEST_REMOTE(suite, cli, PlotDoubleTest);
  RUN_TEST_REMOTE(suite, cli, PlotRandomTest);
}
...
/**
 * @brief Тест со случайными начальными условиями.
 *
 * @param cli Указатель на HTTP клиент.
 */
static void SimpleRandomTest(httplib::Client* cli) {
    ...
    const char* format = R"(
{
  ...
  "id": %d
    ...
    snprintf(command, sizeof(buffer),
        "python \"%s\" HeatConductionPlotter \"%s\" \"%s\"",
...
/**
 * @brief Тест рисовалки со случайными начальными данными.
 *
 * @param cli Указатель на HTTP клиент.
 */
static void PlotRandomTest(httplib::Client* cli) {
